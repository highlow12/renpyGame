label chapter_0:
    ...
    """챙 -"""
    show char n default smi at top
    Character('q') "아까부터 다짜고짜 공격이라니, 너도 제정신은 아니구나?"
    ...
    show char n default bad at top
    Character('q') "그만 좀 해. 나도 상대해 주는 데에 한계가 있다?"

    s "그럼 힘 빼지 말고, 나한테 깔끔하게 잡혀주면 돼."

    show char n default bad at top
    Character('q') "말이 전혀 안 통하는 아가씨네."
    ...
    show char n default def at top
    Character('q') "아~ 뭐지? 무효화 마법을 달고 있는 것도 아닌데? 너 무슨 짓을 한 거야?"

    show char s default bad at top
    s "알려줄 의무가 있나?"

    show char n default def at top
    Character('q') "흠. 괜히, 황실 대표인 게 아닌가 보네. 아님 뭐 특별한 힘이라도 있는 건지···."

    hide char
    """이미 내 존재에 대해 알고 있는 모양이었다. 그리고 그가 궁금해하는 건 내가 아니라, 내 ‘정체’에 관한 것 같았다."""

    show char n default def at top
    Character('q') "아니다. 어차피 감시해야 하는데, 실험체로 두고 보면 되겠네."
